 ...
 * @param {string[]} messageArray The specific message id of the message to be
 *     deleted
 * @param {boolean} onlyLocal If it should only delete locally (message remains
 *     on the other recipienct's phone). Defaults to false.
...
export function deleteMessages(chatId, messageArray, onlyLocal, done) {
  ...
  let messagesToDelete =
      messageArray
          .map((msgId) => typeof msgId == 'string' ? window.Store.Msg.get(msgId)
                                                   : msgId)
          .filter((x) => x);
